...
export default function DueLocations() {
  ...
  const fetchLocations = async () => {
    ...
    try {
      ...
      const trialLocations = allLocations
        .filter(loc => loc.subscriptionStatus === 'TRIAL')
  ...
  const summaryData = useMemo(() => {
    return {
      ...
      trialSubscriptions: filteredLocations.length,
    };
  }, [filteredLocations]);
      ...
      <div className="flex-1 space-y-4 md:p-8 pt-4 pb-4">
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-col md:flex-row justify-between gap-4">
            <h3 className="text-xl font-semibold">Locations with Trial Subscriptions</h3>
            <div className="flex flex-col md:flex-row gap-2">
              ...
              <ProtectedComponent requiredPermissions={['export:report']}>
                <ExportButton 
                  ...
                  exportOptions={{
                    filename: "Trial Locations Report",
                    includeTimestamp: true,
                  }}
                  filterDescription={getFilterDescription()}
                  summaryData={{
                    total: summaryData.total,
                    "Trial Subscriptions": summaryData.trialSubscriptions,
                  }}
                />
              </ProtectedComponent>
            </div>
        ...
        {!error && filteredLocations.length === 0 && (
          <Alert>
            <AlertDescription>
              No trial locations found for the selected date range.
            </AlertDescription>
          </Alert>
        )}
