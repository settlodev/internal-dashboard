...
'use client'
import { useState, useEffect } from 'react';
import { FileText, DollarSign, CreditCard, Wallet, RefreshCw, AlertCircle, Calendar, Database, PlayCircle } from 'lucide-react';
import {financialReconciliationReport, packageContributionReport} from "@/lib/actions/report";
import {InvoicePaymentsSummary} from "@/types/report/type";

// Add interface for enhanced payment data
interface EnhancedInvoicePayment {
    percentage: number;
    provider: string;
    amount: number;
    displayName: string;
...
export default function FinancialPage() {
    ...
    // Helper function to enhance payment data with display properties
    const enhancePaymentData = (payment: any): EnhancedInvoicePayment => {
        const providersConfig: Record<string, { displayName: string; icon: any; colorClass: string }> = {
            'SELCOM': {
                displayName: 'Selcom Payments',
                icon: CreditCard,
            ...
            'MIGRATION': {
                displayName: 'Migration Payments',
                icon: Database,
                colorClass: 'bg-blue-100 text-blue-600'
            },
            'SETTLO_TRIAL': {
                displayName: 'Settlo Trial',
                icon: PlayCircle,
                colorClass: 'bg-yellow-100 text-yellow-600'
            }
        };

        const config = providersConfig[payment.provider] || {
        ...
        return {
            ...
            status: payment.amount > 0 ? 'Verified' : 'No Activity'
        };
    };
    ...
    // Get all payment providers with enhanced data
    const getAllPayments = (): EnhancedInvoicePayment[] => {
        if (!apiData?.invoicePayments) return [];
        return apiData.invoicePayments.map(payment => enhancePaymentData(payment));
    };

    // Get individual payment provider (for backwards compatibility in summary cards)
    const getPaymentByProvider = (provider: string) => {
        if (!apiData?.invoicePayments) return { amount: 0, percentage: 0 };
        const payment = apiData.invoicePayments.find((p: any) => p.provider === provider);
        return payment || { amount: 0, percentage: 0 };
    };
    getPaymentByProvider('SELCOM');
    getPaymentByProvider('MANUAL');
    getPaymentByProvider('MIGRATION');
    getPaymentByProvider('SETTLO_TRIAL');
    const totalAmount = apiData?.totalAmount || 0;
    const totalVerified = apiData?.invoicePayments?.reduce((sum, payment) => sum + payment.amount, 0) || 0;
    const variance = totalAmount - totalVerified;
    const hasData = apiData && totalAmount > 0;

    // Get active payments (with amount > 0)
    const activePayments = getAllPayments().filter(payment => payment.amount > 0);
        ...
        <div className="min-h-screen bg-gray-50 p-6">
            <div className="max-w-7xl mx-auto">
                ...
                {loading ? (
                    ...
                    <>
                        {/* Summary Cards - Show all active payment methods */}
                        <div className={`grid grid-cols-1 ${activePayments.length === 4 ? 'md:grid-cols-4' : 'md:grid-cols-3'} gap-6 mb-6`}>
                            {activePayments.map((payment, index) => {
                                ...
                                return (
                                    <div key={index} className="bg-white rounded-lg shadow-sm p-6 hover:shadow-md transition-shadow">
                                        <div className="flex items-center justify-between">
                                            <div>
                                                <p className="text-sm text-gray-600 mb-1">{payment.displayName}</p>
                                                <p className="text-2xl font-bold text-gray-900">
                                            ...
                                            <div className={payment.colorClass.replace('text-', 'bg-').replace('600', '100') + ' p-3 rounded-full'}>
                                                <IconComponent className={payment.colorClass} size={24} />
                                            </div>
                                        </div>
                                    </div>
                                );
                            ...
                            <div className="overflow-x-auto">
                                <table className="w-full">
                                        ...
                                        return (
                                            <tr key={index} className="border-b border-gray-100 hover:bg-gray-50">
                                                <td className="py-3 px-4 text-gray-900 flex items-center gap-2">
                                                    <IconComponent size={16} className={payment.colorClass} />
                                                    {payment.displayName}
                                                </td>
                                                <td className="py-3 px-4 text-right font-medium text-gray-900">
                                                    {formatCurrency(payment.amount)}
                                                ...
                                                <td className="py-3 px-4 text-right">
                                                    <span className={`${payment.amount > 0 ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'} text-xs px-2 py-1 rounded`}>
                                                        {payment.status}
                                                    </span>
                                                </td>
                                            </tr>
                                    ...
                                    <tr className="bg-gray-50 font-semibold">
                                        ...
                                        <td className="py-3 px-4 text-right text-gray-600">
                                            {getAllPayments().reduce((sum, payment) => sum + payment.percentage, 0).toFixed(1)}%
                                        </td>
                                        <td className="py-3 px-4"></td>
                                    </tr>
                                    </tbody>
