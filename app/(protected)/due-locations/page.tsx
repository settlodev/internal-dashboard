...
export default function DueLocations() {
  ...
  const fetchLocations = async () => {
    ...
    try {
      ...
      const dueLocations = allLocations
        .filter(loc => loc.subscriptionStatus === 'DUE')
        .sort((a, b) => new Date(a.dateCreated).getTime() - new Date(b.dateCreated).getTime());
      
      setLocations(dueLocations);
    } catch (error) {
  ...
  const summaryData = useMemo(() => {
    return {
      ...
      dueSubscriptions: filteredLocations.length,
    };
  }, [filteredLocations]);
      ...
      <div className="flex-1 space-y-4 md:p-8 pt-4 pb-4">
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-col md:flex-row justify-between gap-4">
            <h3 className="text-xl font-semibold">Locations with Due Subscriptions</h3>
              ...
              <ProtectedComponent requiredPermissions={['export:report']}>
                <ExportButton 
                  ...
                  summaryData={{
                    ...
                    "Due Subscriptions": summaryData.dueSubscriptions,
                  }}
                />
              </ProtectedComponent>
            </div>
        ...
        {!error && filteredLocations.length === 0 && (
          <Alert>
            <AlertDescription>
              No due locations found for the selected date range.
            </AlertDescription>
          </Alert>
        )}
