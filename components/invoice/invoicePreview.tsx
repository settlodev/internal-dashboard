...
interface InvoicePreviewProps {
  ...
  const InvoicePreview: React.FC<InvoicePreviewProps> = ({ formData }) => {
    ...
    const ownerLocation = typeof formData.owner === 'object' && formData.owner?.countryName 
      ...
      // Get device details
      const deviceName = typeof formData.device === 'object' && formData.device 
        ? formData.device.brand + '-' + formData.device.device_type
        : '';
        const devicePrice = typeof formData.device === 'object' && formData.device 
        ? formData.device.selling_price 
        : 0;
  
  // Calculate some mock values for the preview
  const unitPrice = devicePrice || 0;
  const quantity = formData.quantity || 0;
  const subtotal = unitPrice * quantity;
  const tax = subtotal * 0.18; // 10% tax for example
      ...
      <Card className="bg-white shadow-md">
        ...
        <CardContent className="pt-6">
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  <tr className="border-b border-gray-200">
                    ...
                    <td className="text-right py-2 px-1">{formatCurrency(unitPrice)}</td>
                    <td className="text-right py-2 px-1">{formatNumber(quantity)}</td>
                    <td className="text-right py-2 px-1">{formatCurrency(subtotal)}</td>
                  </tr>
                </tbody>
              </table>
            </div>
          ...
          <div className="flex items-center mt-6">  
            <p className="text-sm mt-1 text-gray-600">TAX IDENFICATION NUMBER(TIN):</p>
          </div>
